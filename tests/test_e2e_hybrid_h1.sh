...
#
# test_e2e_hybrid_h1.sh — Netns end-to-end test for hybrid mode H1
# (ingress classifier + per-lane counters, no TCP-lane transport yet).
#
# What this verifies (per phase, fresh server+client each):
#   Phase 1 [Hybrid] Enabled=true / Tcp=stream:
#     - tunnel establishes and traffic works (ping + TCP transfer) — the
#       behavior-unchanged evidence: H1 classifies for COUNTERS ONLY, every
#       packet still travels the existing CONNECT-IP RAW/DGRAM path.
#     - the client [STATUS] line (platform_linux.c, fires every 30s in
#       ESTABLISHED; ends with "lanes tcp/dgram/raw=<n>/<n>/<n>") reports a
#       NONZERO tcp count (the TCP transfer was classified MQVPN_LANE_TCP)
#       and dgram+raw > 0 (ping is ICMP -> RAW; UDP, if any, -> DGRAM).
...
# Listener binds inside NS_SERVER; client pushes ~64KB and closes on EOF (-N).
run_inner_traffic() {
    ...
    local lpid=$!
    sleep 1
    if ! head -c 65536 /dev/zero | ip netns exec "$NS_CLIENT" \
            timeout 15 nc -N "$TUNNEL_SERVER_IP" "$INNER_TCP_PORT" \
            >/dev/null 2>&1; then
...
echo ""
echo "=== Phase 1: hybrid ON, Tcp=stream (TCP lane counted) ==="
hybrid_run "$INI_P1" "$SERVER_LOG_P1" "$CLIENT_LOG_P1"

if run_inner_traffic; then
    echo "PASS: traffic works with hybrid ON / Tcp=stream (behavior unchanged)"
else
    echo "FAIL: traffic failed with hybrid ON / Tcp=stream"
    fail=1
fi

# RACE PIN: demand a NONZERO tcp field — an early [STATUS] tick may
# legitimately report 0 before the traffic above ran.
if wait_for_log "$CLIENT_LOG_P1" 'lanes tcp/dgram/raw=[1-9][0-9]*/' 40; then
    ...
    if [ "$((LANE_DGRAM + LANE_RAW))" -gt 0 ]; then
        echo "PASS: dgram+raw > 0 (ping->raw / udp->dgram counted)"
    else
        echo "FAIL: dgram+raw == 0 despite ping traffic (dgram=$LANE_DGRAM raw=$LANE_RAW)"
        fail=1
    fi
else
