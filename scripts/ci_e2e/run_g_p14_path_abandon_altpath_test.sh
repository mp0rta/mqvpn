...
#
# When the primary path goes down, the client must emit PATH_ABANDON for
# path_id=0 routed via an alternate path (path_id=1) so the server can
# release resources without waiting for idle timeout (~30s).
#
# Topology: standard dual-path veth (-gp14 suffix).
#
# Procedure:
#   1. Establish 2-path tunnel.
#   2. Bring veth-a0 down inside client netns.
#   3. Wait <=10s for server to log path_abandon receipt for path_id=0.
#
