...
| `H3BodyBufPerStream` | Buffered HTTP/3 DATA payload one request may hold, in bytes. Sets `xqc_conn_settings_t.max_body_buf_per_stream`. Applies to client **and** server, unlike `RecvRateLimit`. Maximum `4294967295`; larger values are rejected with a warning and the key falls back to `0`. Read the notes under this table before setting it | `0` (xquic's own default) |
| `BlockedBufPerStream` | Per-stream QPACK decode-blocked buffer; sets `xqc_conn_settings_t.max_blocked_buf_per_stream`. **Exceeding it closes the connection** — read the note below before setting it. `0` means 1 MiB on a server and unbounded on a client. Maximum `4294967295`; a larger value is logged as invalid and ignored, which on a client leaves it unbounded | `0` |
| `BlockedBufPerConn` | The same, across one connection; sets `xqc_conn_settings_t.max_blocked_buf_per_conn`, with the same connection-closing enforcement. `0` means 8 MiB on a server and unbounded on a client. Maximum `4294967295`; a larger value is logged as invalid and ignored, which on a client leaves it unbounded | `0` |
| `MaxRecvWindow` | Ceiling on the **per-stream** receive window in bytes; sets `xqc_conn_settings_t.max_recv_window`. Not the connection window (that is `RecvRateLimit`). Maximum `4294967295`; a larger value is logged as invalid and ignored | `0` (xquic's own default) |

In JSON, the section is an `"advanced"` object with snake_case keys (`recv_rate_limit`, `udp_gso`, `udp_gro`, `h3_body_buf_per_stream`, `blocked_buf_per_stream`, `blocked_buf_per_conn`, `max_recv_window`).

**`H3BodyBufPerStream` and `MaxRecvWindow` need an xquic that has the matching setting.** They map to `xqc_conn_settings_t.max_body_buf_per_stream` and `.max_recv_window`, neither of which is in the xquic this repository pins today. Until an xquic that carries them is pinned, **these two keys cannot be used**: the build probes for each field, and setting a key whose field is missing is **refused at startup** with an error naming it, rather than accepted and ignored. `0` is always accepted on any build, because `0` means "leave the library's own default alone" and needs no field. `BlockedBufPerStream` and `BlockedBufPerConn` are the two that work with the xquic this repository pins today.

**`BlockedBufPerStream` and `BlockedBufPerConn` do not backpressure — they close the connection.** When the QPACK decode-blocked buffer passes either limit, mqvpn's HTTP/3 connection is torn down with `H3_EXCESSIVE_LOAD`, and on this transport that is the whole tunnel: every TCP flow riding it goes with it and the client reconnects. The number you write is how much decode-blocked data a peer may send before that happens.

**Leaving both at `0` is a different choice on each side.** `0` gets you the library's internal 1 MiB / 8 MiB on a **server** — `xqc_server_set_conn_settings()` substitutes them — and no bound at all on a **client**, whose settings are passed through as given.
