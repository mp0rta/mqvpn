...
| `TcpMaxFlows` | Concurrent TCP-lane flow cap. Client: local flow table (over-cap SYNs fall back to RAW). Server: per client session (over-cap SYNs get HTTP `503`). This caps a flow *count*, but it is also what bounds the TCP lane's memory: the lane-owned per-flow uplink queue has a hard ceiling of ~2.25 MiB, so the client-side worst case is `TcpMaxFlows` × 2.25 MiB (≈ 576 MiB at the default 256, ≈ 9.0 GiB at 4096). Size it from the box's RAM; over-cap SYNs degrade to the RAW lane rather than failing. On the client the value is additionally clamped to half the lwIP TCP pcb pool, which differs per build profile — `4096` on desktop/router builds (Linux, Windows, macOS), `256` on Android, `64` with the [iOS lwIP profile](./hybrid-mode#ios-builds) — and the clamp is logged. The other pool half backs the TIME_WAIT/LAST_ACK/CLOSING pcbs the flow table has stopped counting; without that headroom lwIP would run its pcb pool dry and start evicting **established** inner connections to admit new ones, instead of the lane's own cap refusing the SYN first | client + server | `256` |
