...
標準 CLI では、起動時に `--path` フラグでパスを指定します。その後はプラットフォーム層がそれらのインターフェースを実行中も監視し（Linux は netlink、macOS は `PF_ROUTE`）、インターフェースがダウンするとパスを自動で削除、使用可能なアドレスを持って復帰するとソケットを再作成してパスを自動で再追加します（定期リカバリタイマーによるバックストップ付き）。未実装なのは、起動時に登録していない全く新規のインターフェースの自動発見のみで、監視対象は `--path` で指定したインターフェースに限られます。
