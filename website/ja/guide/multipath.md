...
**こんなときに選ぶ**: トンネル内でさらに QUIC コネクションを張る用途——
クラウドサービスへの HTTP/3、SRT による映像配信、WireGuard セッションなど。
この種の UDP プロトコルはパケットの並び替えに弱く、mqvpn が遅延差のある
パスにパケットを散らすと、中の通信がそれを「パケットロス」と勘違いして
速度を落としてしまい、結果としてスループットが大きく下がります。各
コネクションを 1 本のパスに固定しておけばこの落とし穴を避けられます。

**そのままの `wlb` で十分なケース**: 日常的なトラフィックで、小さな UDP
フローがたくさん混在する構成——ビデオ通話（WebRTC）、オンラインゲーム、
普通の Web 閲覧、DNS など。これらはアプリ側でもともと多少の並び替えに
耐えられるようになっていて、mqvpn 側でパケットを散らしてあげた方が
複数回線を合算した帯域を活かせます。実装上の事情もあって、mqvpn が
追跡できる UDP コネクション数には上限があるため、たとえば DNS サーバの
ように毎秒大量の短命フローが発生する用途では、古いフローの追跡が
こぼれて固定の効果が薄くなります。そうした構成では `wlb` の方が無難です。

ざっくり言えば、「順序を気にする長命なコネクション数本」がトンネル内
トラフィックの大半を占めるなら `wlb_udp_pin`、それ以外は `wlb` のまま、
という選び分けで OK です。
