...
トンネルの中でさらに QUIC コネクションを張るような用途——クラウドサービスへの
HTTP/3、SRT による映像配信、WireGuard セッションなど——で効きます。この手の
UDP プロトコルはパケットの並び替えに弱く、遅延差のある複数パスにパケットを
散らされると、内側の通信が並び替えをロスと誤認して速度を落としてしまい、
結果としてスループットが大きく下がります。各コネクションを 1 本のパスに
固定しておけばこの問題を避けられます。
