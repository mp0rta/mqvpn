...
**`H3BodyBufPerStream` と `MaxRecvWindow` には対応する設定を持つ xquic が必要です。** この 2 つは `xqc_conn_settings_t.max_body_buf_per_stream` と `.max_recv_window` に対応しますが、どちらも現在このリポジトリが pin している xquic にはありません。それらを持つ xquic が pin されるまで、**この 2 つのキーは使えません**。ビルド時にそれぞれのフィールドの有無を検出し、フィールドがないキーを設定した場合、そのキーは受け付けられて無視されるのではなく、キー名を示すエラーとともに**起動時に拒否**されます。`0` はどのビルドでも受け付けられます。`0` は「ライブラリ自身の既定値に触らない」という意味で、フィールドを必要としないためです。このリポジトリが現在 pin している xquic で実際に効くのは `BlockedBufPerStream` と `BlockedBufPerConn` の 2 つです。

**`BlockedBufPerStream` と `BlockedBufPerConn` はバックプレッシャではありません — コネクションを切断します。** QPACK デコードブロックバッファがどちらかの上限を超えると、mqvpn の HTTP/3 コネクションは `H3_EXCESSIVE_LOAD` で切断されます。このトランスポートではそれはトンネル全体を意味し、乗っている TCP フローが一斉に死に、クライアントは再接続します。ここに書く数字は、そうなるまでにピアが送ってよいデコードブロックデータ量です。

**両方を `0` のままにするのも 1 つの選択であり、その意味は左右で異なります。** `0` は**サーバ**ではライブラリ内部の 1 MiB / 8 MiB を意味し（`xqc_server_set_conn_settings()` が代入します）、**クライアント**では上限がありません。クライアント側の設定はそのまま渡されます。
