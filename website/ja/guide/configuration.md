...
| `TcpMaxFlows` | 同時 TCP レーンフロー数の上限。client: ローカルフローテーブル（超過 SYN は RAW にフォールバック）。server: クライアントセッション単位（超過 SYN は HTTP `503`）。client 側ではさらに lwIP TCP pcb プールの半分に clamp される — デフォルトビルドで `256`、[モバイル lwIP プロファイル](./hybrid-mode)で `64` — clamp 時はログに出力 | client + server | `256` |
