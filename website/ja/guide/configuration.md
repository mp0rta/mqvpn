...
| `TcpMaxFlows` | 同時 TCP レーンフロー数の上限。client: ローカルフローテーブル（超過 SYN は RAW にフォールバック）。server: クライアントセッション単位（超過 SYN は HTTP `503`）。client 側ではさらに lwIP TCP pcb プールの半分に clamp される — デフォルトビルドで `256`、[iOS lwIP プロファイル](./hybrid-mode#ios-ビルド)で `64` — clamp 時はログに出力。プールの残り半分はフローテーブルが数えない TIME_WAIT / half-open の pcb 用で、この余裕がないと pcb 枯渇時に新規 SYN が明示的な reject（RST）ではなく無言でハングするため | client + server | `256` |
