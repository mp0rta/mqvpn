...
typedef void (*mqvpn_tun_output_fn)(
    const uint8_t *pkt, size_t len, void *user_ctx);

typedef void (*mqvpn_tunnel_config_ready_fn)(
    const mqvpn_tunnel_info_t *info, void *user_ctx);

typedef void (*mqvpn_send_packet_fn)(
    mqvpn_path_handle_t path, const uint8_t *pkt, size_t len,
    const struct sockaddr *peer, socklen_t peer_len, void *user_ctx);

typedef void (*mqvpn_tunnel_closed_fn)(
    mqvpn_error_t reason, void *user_ctx);

typedef void (*mqvpn_ready_for_tun_fn)(void *user_ctx);

typedef void (*mqvpn_state_changed_fn)(
    mqvpn_client_state_t old_state, mqvpn_client_state_t new_state,
    void *user_ctx);

typedef void (*mqvpn_path_event_fn)(
    mqvpn_path_handle_t path, mqvpn_path_status_t status, void *user_ctx);

typedef void (*mqvpn_mtu_updated_fn)(int mtu, void *user_ctx);

typedef void (*mqvpn_log_fn)(
    mqvpn_log_level_t level, const char *msg, void *user_ctx);
...
MQVPN_API int mqvpn_config_set_scheduler(mqvpn_config_t *cfg,
                                          mqvpn_scheduler_t sched);
MQVPN_API int mqvpn_config_set_log_level(mqvpn_config_t *cfg,
                                          mqvpn_log_level_t level);
MQVPN_API int mqvpn_config_set_multipath(mqvpn_config_t *cfg, int enable);
MQVPN_API int mqvpn_config_set_reconnect(mqvpn_config_t *cfg,
                                          int enable, int interval_sec);
...
MQVPN_API int mqvpn_config_set_subnet(mqvpn_config_t *cfg,
                                       const char *cidr);
MQVPN_API int mqvpn_config_set_subnet6(mqvpn_config_t *cfg,
                                        const char *cidr6);
MQVPN_API int mqvpn_config_set_tls_cert(mqvpn_config_t *cfg,
                                         const char *cert, const char *key);
MQVPN_API int mqvpn_config_set_max_clients(mqvpn_config_t *cfg, int max);

/* ─── Client API ─── */

MQVPN_API mqvpn_client_t *mqvpn_client_new(
...
MQVPN_API mqvpn_path_handle_t mqvpn_client_add_path_fd(
    mqvpn_client_t *client, int fd,
    const mqvpn_path_desc_t *desc);

MQVPN_API int mqvpn_client_remove_path(mqvpn_client_t *client,
                                        mqvpn_path_handle_t path);

MQVPN_API int mqvpn_client_set_tun_active(mqvpn_client_t *client,
                                            int active, int tun_fd);

/* Feed data from platform into the engine */
MQVPN_API int mqvpn_client_on_tun_packet(mqvpn_client_t *client,
                                           const uint8_t *pkt, size_t len);

MQVPN_API int mqvpn_client_on_socket_recv(
    mqvpn_client_t *client, mqvpn_path_handle_t path,
    const uint8_t *pkt, size_t len,
...
/* Query state */
MQVPN_API mqvpn_client_state_t mqvpn_client_get_state(
    const mqvpn_client_t *client);

MQVPN_API int mqvpn_client_get_stats(const mqvpn_client_t *client,
                                       mqvpn_stats_t *out);

MQVPN_API int mqvpn_client_get_paths(const mqvpn_client_t *client,
...
MQVPN_API mqvpn_server_t *mqvpn_server_new(
    const mqvpn_config_t *cfg,
...
MQVPN_API int mqvpn_server_on_socket_recv(
    mqvpn_server_t *server,
    const uint8_t *pkt, size_t len,
    const struct sockaddr *peer, socklen_t peer_len);

MQVPN_API int mqvpn_server_on_tun_packet(mqvpn_server_t *server,
                                            const uint8_t *pkt, size_t len);
