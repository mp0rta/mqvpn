...
- 回線断への耐性: 配信中に片方の回線を 30 秒間遮断すると、direct の配信は約 34 秒止まった。mqvpn は切断 0 回、約 1 秒の乱れだけで配信が続いた。
...
3 回の繰り返しすべてで切断 0 回、停止 1.0 秒、7.9 Mbps という同じ結果になった。
...
乱れは約 1 秒で、3 回の繰り返しすべてで再現した。

![ライブからの遅れの推移 (条件 3、代表 1 回分)](../../bench_results/rtmp/charts/lag_R3_rep1.png)

mqvpn の線が途切れていないのは、接続が一度も切れていないからである。
表の停止 1.0 秒は受信側で測ったもので、回線を切り替える瞬間にデータの到達が約 1 秒途切れたことを指す (送信側のグラフには遮断時点のわずかな乱れとして現れる)。
接続の切断とデータ到達の途切れは別の事象であり、direct は両方が 30 秒級、mqvpn は切断 0 回と到達の途切れ 1 秒である。
...
[^deadair]: 受信側 (RTMP サーバ) の録画ファイルのバイト増加を 250 ms 間隔で監視し、1 秒を超えて増加が止まった時間の合計を配信停止と判定した。送信ソフトが動いていても配信先にデータが届いていなければ停止に数える。定義の詳細は `benchmarks/rtmp_analyze.py` を参照。
