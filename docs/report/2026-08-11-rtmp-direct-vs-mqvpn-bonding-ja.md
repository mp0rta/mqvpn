...
| 回線復旧から配信再開までの時間 | 2.2〜4.0 秒 | 該当なし (停止しない) |
...
配信ソフトは再接続を試み続けるが、同じ回線しかないため回線自体が復旧するまで失敗し続ける。
停止時間が遮断時間より長い 32.9 秒になるのはこのためで、内訳は遮断そのものの 30 秒に、復旧後に再接続が成功して最初の映像が届くまでの 2〜4 秒を加えたものである (切断の検知は遮断中に済むため、ここには加算されない)。
