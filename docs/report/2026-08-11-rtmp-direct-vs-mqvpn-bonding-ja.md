...
- 不安定な回線での安定性: モバイル回線を模した間欠的なパケット損失の下で、direct は 60 秒間に平均 4 回切断し、実質配信不能 (0.9 Mbps) になった。mqvpn は切断 0 回、遅れ 1 秒、7.9 Mbps を維持した。
- 回線断への耐性: 配信中に片方の回線を 30 秒間遮断すると、direct の配信は約 34 秒止まった。mqvpn は切断 0 回で、測定できる水準の途切れなしに配信が続いた。
...
- **配信停止時間**：配信先にデータが届かなかった時間の合計 (視聴者には映像の停止として見える)[^deadair]
...
|---|---|---|
| 切断回数 | 4.0 回 | 0 回 |
| 配信停止時間 (60 秒中) | 43.7 秒 | 1.0 秒 |
| ライブからの遅れ (最大) | 43.0 秒 | 1.0 秒 |
| 届いたビットレート | 0.9 Mbps | 7.9 Mbps |

TCP はパケット損失が続くと再送を重ねて送信が詰まる。
direct ではこの詰まりが繰り返し切断判定 (10 秒) を超え、60 秒のうち 44 秒間、配信先に何も届かなかった。
mqvpn では、損失からの回復をトンネルが回線ごとに引き受けるため、配信ソフトには安定した通り道だけが見える。
3 回の繰り返しすべてで切断 0 回、7.9 Mbps という同じ結果になった。
表の停止 1.0 秒は損失によるものではなく、配信開始直後にどの構成でも 1 回現れる立ち上がりの空白である[^deadair]。
損失下の 60 秒間について見ると、データ到達の途切れは最大でも 0.5 秒程度にとどまった。
...
差はビットレートではなく、34 秒の停止と 8.7 回の切断に現れている。
...
表の停止 1.0 秒も回線切替によるものではない。
生データで位置を確認すると、この空白は配信開始直後の立ち上がり (どの構成でも現れる[^deadair]) にあり、遮断の前後では、バイト到達の間隔が監視の 1 サンプル分 (250 ms) を超えることは 3 回の繰り返しを通じて一度もなかった。
...
[^deadair]: 受信側 (RTMP サーバ) の録画ファイルのバイト増加を 250 ms 間隔で監視し、1 秒を超えて増加が止まった時間の合計を配信停止と判定した。送信ソフトが動いていても配信先にデータが届いていなければ停止に数える。なお、配信開始直後にはエンコーダが最初の映像を送り出すまでの約 1 秒の空白がどの構成でも 1 回現れ、これも停止時間に含まれる (mqvpn の各条件の 1.0 秒、および条件 1 の direct の 1.3 秒はこの立ち上がり分である)。定義の詳細は `benchmarks/rtmp_analyze.py` を参照。
