...
- **picoquic (the inner of this study, `e652e454`) — no dynamic expansion.** Fixed `delta_seq >= 3` ([`loss_recovery.c#L636`](https://github.com/private-octopus/picoquic/blob/e652e454b40ff94d7a0372d537fdf176d55b61f1/picoquic/loss_recovery.c#L636)). Spurious retransmissions are only recorded as telemetry (`picoquic_path_quality_t::max_reorder_gap`, [`picoquic.h#L1604`](https://github.com/private-octopus/picoquic/blob/e652e454b40ff94d7a0372d537fdf176d55b61f1/picoquic/picoquic.h#L1604), populated at [`quicctx.c#L3596`](https://github.com/private-octopus/picoquic/blob/e652e454b40ff94d7a0372d537fdf176d55b61f1/picoquic/quicctx.c#L3596)) and never fed back into the threshold. The collapses in §4 are this behaviour.
- **Google QUICHE (Chrome / Cronet) — dynamic expansion.** `use_adaptive_reordering_threshold_ = true` by default ([`general_loss_algorithm.h#L121`](https://github.com/google/quiche/blob/f001eed73bcff9389be32a36047e8945fba32553/quiche/quic/core/congestion_control/general_loss_algorithm.h#L121)); `SpuriousLossDetected()` grows `reordering_threshold_` on each spurious detection. Initial value `kDefaultPacketReorderingThreshold = 3` ([`quic_constants.h#L288`](https://github.com/google/quiche/blob/f001eed73bcff9389be32a36047e8945fba32553/quiche/quic/core/quic_constants.h#L288)).
...
- picoquic `e652e454`: `loss_recovery.c#L636` (fixed `delta_seq >= 3`) / `picoquic.h#L1604` (`max_reorder_gap` telemetry field) / `quicctx.c#L3596` (populate)
- Google QUICHE `f001eed`: `general_loss_algorithm.h#L121` (`use_adaptive_reordering_threshold_ = true`) / `quic_constants.h#L288` (`kDefaultPacketReorderingThreshold = 3`)
